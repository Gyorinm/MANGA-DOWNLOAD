...
class OlympusStaffSource implements MangaSource {
  ...
  Future<Manga?> _directSlugSearch(String query) async {
    final base = query.trim().toLowerCase().replaceAll(RegExp(r'[^a-z0-9]+'), '-');
    var slug = base;
    while (slug.startsWith('-')) slug = slug.substring(1);
    while (slug.endsWith('-')) slug = slug.substring(0, slug.length - 1);
    if (slug.isEmpty) return null;

    // نحاول عدة أشكال للـ slug: الأصلي، وبدون كلمات وصل، ومع in- بدلاً من -in-
    final candidates = <String>{
      slug,
      slug.replaceAll(RegExp(r'-(?:in|of|the|a|an)-'), '-'),
      slug.replaceAll('-in-', '-'),
    }.where((s) => s.isNotEmpty).toList();

    for (final s in candidates) {
      try {
        final document = await _document('$_base/series/$s');
        final title = cleanText(document.querySelector('h1, .author-info-title h6, .title')?.text ?? '');
        if (title.isEmpty || document.querySelector('.chapter-card, .enhanced-chapters-section') == null) continue;
        final cover = imageUrl(_base, document.querySelector('img[alt="Manga Image"], img.shadow-sm, .text-right img'));
        return Manga(sourceId: id, remoteId: s, title: title, remoteCoverUrl: cover.isEmpty ? null : cover);
      } catch (_) {
        continue;
      }
    }
