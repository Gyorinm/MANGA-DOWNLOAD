...
class StarzMangaSource implements MangaSource {
  ...
  Future<Manga?> _directSlugSearch(String query) async {
    ...
    // نحاول عدة أشكال للـ slug وعدة مسارات للموقع
    final slugVariants = <String>{
      slug,
      slug.replaceAll(RegExp(r'-(?:in|of|the|a|an)-'), '-'),
      slug.replaceAll('-in-', '-'),
    }.where((s) => s.isNotEmpty).toList();

    final prefixes = ['manga', 'comics', 'manhwa'];

    for (final s in slugVariants) {
      for (final prefix in prefixes) {
        try {
          final document = await _document('$_base/$prefix/$s/');
          if (!_looksLikeDetail(document)) continue;
